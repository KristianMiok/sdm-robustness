#!/bin/bash
#SBATCH --job-name=mx_Procambarus_clarkii_native_lowacc_combined_L10
#SBATCH --partition=gpu
#SBATCH --qos=normal
#SBATCH --time=12:00:00
#SBATCH --cpus-per-task=4
#SBATCH --mem-per-cpu=4000M
#SBATCH --gres=gpu:1
#SBATCH --output=logs/slurm/%x_%j.out
#SBATCH --error=logs/slurm/%x_%j.err

set -euo pipefail

cd /ceph/hpc/home/miokk/sdm-robustness
module load Python/3.12.3-GCCcore-13.3.0
source .venv/bin/activate

python scripts/run_task5_entity.py \
  --entity "Procambarus clarkii (native)" \
  --data-path /ceph/hpc/home/miokk/sdm-robustness/data/combined_data_true_master.csv \
  --output-dir results/task5_execution/Procambarus_clarkii_native_maxent_lowacc_combined_L10 \
  --skip-benchmark \
  --algorithms maxent \
  --axes lowacc \
  --tracks combined \
  --levels 10 \
  --n-replicates-default 30 \
  --n-replicates-low-levels 50 \
  --n-experiment 660
